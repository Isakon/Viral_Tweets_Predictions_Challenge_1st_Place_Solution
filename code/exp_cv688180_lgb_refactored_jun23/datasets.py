...
# def add_date_feats_resample_monthly(df, impute_nulls=False, impute_value=0):
...
#     # stats_dict = {feat: stats for feat in cols_resample}
#     for freq_ in date_freqs:
#         for stat_ in stats:
#             df.set_index('date', inplace=True)
#             g = (df.groupby('tweet_user_id').resample(freq_, closed='left')
...
#             #df = df.reset_index().merge(g, how='left', on='tweet_user_id')
#             df = df.reset_index().merge(g, how='left', on='tweet_user_id')
#             # df.loc[:,g.columns] = df.loc[:,g.columns].fillna(impute_value)
#             # df.set_index('date', inplace=True)
#
#     df.reset_index(drop=False, inplace=True)
#
...
# def add_date_feats_old_rolling(df, impute_nulls=False, impute_value=0):
#     # todo reinstate index - recover df sorted by date
#     #
#     # columns_drop = [
#     #     "tweet_topic_ids",
...
#     df.set_index('date', inplace=True)
#     cols_tweets_counts = ['tweet_hashtag_count',  'tweet_url_count',  'tweet_mention_count']
#     rolling_days = ['10D', '30D', '69D', '90D', '180D', '365D', '730D', '1000D']  #, '2000D', '3000D']
#     for col_ in tqdm(cols_tweets_counts):
#         # mean_ = train.groupby('tweet_user_id')[col_].mean().reset_index()
#         # sum_ = train.groupby('tweet_user_id')[col_].sum().reset_index()
#         stats = ['mean', 'max', 'min', 'median', 'std']
#         for days_ in rolling_days:
#             for per_ in ['before']:  # ,'center']:  # 'center' - include future dates
#                 # todo per_ = 'center'. 'before' takes on previous periods stats
#                 # todo 'center' with .shift change dates, so impossible proper merge g with df
#                 # https://stackoverflow.com/questions/47014765/use-center-in-pandas-rolling-when-using-a-time-series
#                 # todo: use python 3.8 (pandas 1.3.x). There one can use center' option in rolling
#                 # g = (df.groupby('tweet_user_id').filter(lamda x:
#                 #                             x['date'] ...)
#                 g = (df.groupby('tweet_user_id')[col_].rolling(days_, min_periods=1, closed='left')
#                         .agg(stats)
#                         .astype(np.float32)
#                         .reset_index()  # droplevel(0)
#                         .set_index('date'))
#                 if per_ == 'center':
#                     g = g.shift(int(days_[:-1])//2, freq='D')
#                 g = g.reset_index()
#                 g = g.rename(columns ={col: f"{col_}_{per_}days{days_[:-1]}_{col}"
#                                        for col in g.columns if col not in ['tweet_user_id','date']})
#                 g = g.groupby('date').mean().reset_index()
#                 df = df.reset_index().merge(g, how='left', on=['tweet_user_id', 'date'])
#                 if impute_nulls:
#                     df.loc[:,g.columns] = df.loc[:,g.columns].fillna(impute_value)
#                 df.set_index('date', inplace=True)
#
