...
def main(out_dir, cfg):
    ...
    logprint(f"Time to get data {gettime(t_get_data)}")
    ...
    logprint(f"===== Finished training, validating, predicting folds {cfg.folds_to_train} ======")
    ...
    logprint(f"Average score = {average_score*100:.4f}")
    logprint(f"CV score = {cv_score*100:.4f}")
    logprint(f"Total RUNTime = {gettime(t0)}")
    logprint(" ================= END OF LGBM ith BASE MODEL =================")
