...
def run(fold, train, test, feats, feat_idxs):
    ...
    logprint(f"Train set n = {len(train_fold)}, Valid set n = {len(val_fold)}, Num feats = {train_fold.shape[1]}")
    ...
    logprint(f"{time.ctime()} ===== Fold {fold}/{cfg.n_folds-1} starting ======")
    ...
    for epoch in range(cfg.n_epochs):
        ...
        if acc_epoch > best_acc:
            logprint(f'best accuracy ({best_acc:.5f} --> {acc_epoch:.5f}). Saving model ...')
...
def main(out_dir, cfg):
    ...
    print(" ================= END OF DNN3 seed=3 BASE MODEL =================")
