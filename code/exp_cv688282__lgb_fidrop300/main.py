...
def main(out_dir, cfg):
    ...
    # save ['tweet_id','virality'] for stacking used in stack_preds.py
    train[['tweet_id', 'tweet_user_id', 'virality','fold']].to_csv(cfg.train_ids_virality_fold_path, index=False)
    (feat_cols, media_img_feat_cols, text_feat_cols,
     user_des_feat_cols, user_img_feat_cols, feats_some) = get_feat_cols(train)
