...
class ViralDataset(torch.utils.data.Dataset):
    def __init__(self, df: pd.DataFrame, feat_cols: list, feat_idxs: dict, mode: str):
...
class ViralDatasetStack(torch.utils.data.Dataset):
    def __init__(self, X: np.array, y=None, mode='train'):
        self.X = X
        self.mode = mode
...
# class ViralDatasetWithEmbeddings(torch.utils.data.Dataset):
#     def __init__(self, df: pd.DataFrame, feat_cols: list, feat_idxs: dict, mode: str):
#         self.X = df[feat_cols].values  # [:,np.newaxis,:]
#         self.mode = mode
#         self.float_idxs = [c_idx for c_idx in feat_idxs[key] for key in feat_idxs.keys()
#                                                                 if key!='category_feats']
#         self.cat_idxs = feat_idxs['category_feats']
#         assert len(self.float_idxs) + len(self.cat_idxs) == len(feat_cols)
#
#         if mode != 'test':
#             self.targets = df['virality'].values  # [:,np.newaxis]  # - 1
#             # assert np.sum(~df['virality'].isin(list(range(5))))==0
...
#
#     def __getitem__(self, idx):
#         X_floats = X[idx, self.float_idxs]
#         X_cats = X[idx, self.cat_idxs]
#
#         if self.mode=='test':
#             return {'X_floats': torch.tensor(X_floats, dtype=torch.float32),
#                     'X_cats': torch.tensor(X_cats, dtype=torch.long)}
#         else:
#             return {'X_floats': torch.tensor(X_floats, dtype=torch.float32),
#                     'X_cats': torch.tensor(X_cats, dtype=torch.long),
#                     'y_targets': torch.tensor(self.targets[idx], dtype=torch.long)
#                     }
...
def add_date_feats(df):
    ...
    df['user_followers_count_2days'] = (df['user_followers_count'] / df['days_since_tweet']).astype(np.float32)
    df['user_following_count_2days'] = (df['user_following_count'] / df['days_since_tweet']).astype(np.float32)
    df['user_listed_on_count_2days'] = (df['user_listed_on_count'] / df['days_since_tweet']).astype(np.float32)
    df['user_tweet_count_2days'] = (df['user_tweet_count'] / df['days_since_tweet']).astype(np.float32)
    df['tweet_hashtag_count_2days'] = (df['tweet_hashtag_count'] / df['days_since_tweet']).astype(np.float32)
    df['tweet_mention_count_2days'] = (df['tweet_mention_count'] / df['days_since_tweet']).astype(np.float32)
    df['tweet_url_count_2days'] = (df['tweet_url_count'] / df['days_since_tweet']).astype(np.float32)

    # todo not a date related functions:
    df['tweet_mention_count_div_followers'] = df['tweet_mention_count'].divide(df['user_followers_count']+1).astype(np.float32)
    df['tweet_url_count_div_followers'] = df['tweet_url_count'].divide(df['user_followers_count']+1).astype(np.float32)
    df['tweet_hashtag_count_div_followers'] = df['tweet_hashtag_count'].divide(df['user_followers_count']+1).astype(np.float32)
    df['tweet_mention_count_div_followers'] = df['tweet_mention_count'].divide(df['user_followers_count']+1).astype(np.float32)

    df['tweet_mention_count_div_n_tweets'] = df['tweet_mention_count'].divide(df['user_tweet_count']+1).astype(np.float32)
    df['tweet_url_count_div_n_tweets'] = df['tweet_url_count'].divide(df['user_tweet_count']+1).astype(np.float32)
    df['tweet_hashtag_count_div_n_tweets'] = df['tweet_hashtag_count'].divide(df['user_tweet_count']+1).astype(np.float32)
    df['tweet_mention_count_div_n_tweets'] = df['tweet_mention_count'].divide(df['user_tweet_count']+1).astype(np.float32)

    df['tweet_mention_count_div_likes'] = df['tweet_mention_count'].divide(df['user_like_count']+1).astype(np.float32)
    df['tweet_url_count_div_likes'] = df['tweet_url_count'].divide(df['user_like_count']+1).astype(np.float32)
    df['tweet_hashtag_count_div_likes'] = df['tweet_hashtag_count'].divide(df['user_like_count']+1).astype(np.float32)
    df['tweet_mention_count_div_likes'] = df['tweet_mention_count'].divide(df['user_like_count']+1).astype(np.float32)
...
class Features():
    ...
    def get_data_stage2(self, cfg, df):
        ...
        # todo add more columns for frequency
...
def get_folds(cfg, train, default_seed_folds=24):
    ...
    if cfg.seed_folds == default_seed_folds:
        ...
        if cfg.folds_split_method == 'user_viral':
            ...
            print(f"Different folds split with seed={cfg.seed_folds}")
    return train
...
def add_topic_nums2embed(df):
    # and drop the column
    nan_replace = '0'
    # add topic_{i} columns
    topics_df = df['tweet_topic_ids'].fillna(f'[{nan_replace}]')
    topics_df = topics_df.apply(lambda x: list(eval(x)))
    temp_df = pd.DataFrame(topics_df.to_list()).fillna(int(nan_replace)).astype(int)
    temp_df.columns = [f"topic_id_at_pos_{i}" for i in range(temp_df.shape[1])]
    # label encode
    unique_topics = np.sort(np.unique(temp_df.values))
    map_topics = {u: i for (u,i) in zip(unique_topics, range(len(unique_topics)))}
    map_func = lambda x: map_topics[x]
    temp_df.iloc[:] = temp_df.iloc[:].applymap(map_func)
